...
description: 了解如何協助AdobeExperience League說明檔案。
...
從 [Git和GitHub基本介紹](git-fundamentals.md) Git存放庫包含主要分支，以及任何尚未整合至主要的進行中分支。 每當您導入一組邏輯縝密的變更時，最佳作法是建立 *工作分支* 以透過工作流程管理變更。 我們在此將其稱為工作分支，因為這是可反覆查詢/調整變更的工作區，直到變更可整合回主分支為止。

隔離對特定分支進行的相關變更，即可單獨控制並導入這些變更，將其鎖定至發佈週期中的特定發行時間。實際上，存放庫隨便都會有數個工作分支，端視您的工作類型而定。同時存在多個分支是很常見的現象，而每個分支都代表不同專案。

>[!NOTE]
>
>在主分支中進行變更 *不是好做法*. 假設您使用主分支，針對計時功能版本導入一組變更。 您順利完成變更，準備發行版本。在過渡期間，您會收到修正某項內容的緊急請求，因此您變更了主分支中的檔案，然後發佈變更。 這個例子中，您不小心發佈了修正&#x200B;*和*&#x200B;保留要在特定日期發佈的變更。
...
您可將提出的變更合併成新的提取請求 (PR)，新增至目的地存放庫 PR 佇列，再提交變更。只要提取工作分支的變更並合併至其他分支，提取請求即可啟用 GitHub 的協作模型。在大多數情況下，其他分支是主要存放庫中的預設/主分支。
