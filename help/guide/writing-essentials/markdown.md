...
我們目前是將 Adobe 文件內容儲存在 GitHub，而 GitHub 使用的 Markdown 版本稱為 [GitHub Flavored Markdown (GFM)](https://help.github.com/categories/writing-on-github/)，可提供額外功能滿足常見的格式需求。 此外，Adobe 更適度擴充 Markdown 效能，以支援註釋、提示和嵌入影片等特定的說明相關功能。
...
* [將HTML表格轉換為Markdown](https://jmalarcon.github.io/markdowntables/)
...
若連結的是同一存放庫內的文章 (交叉參照)，請使用相對連結。 您可以使用所有相對連結運算元，例如。/ （目前目錄）、../ （上一層目錄）和……/./ （上一層目錄）。
...
原生 Markdown 不會轉譯嵌入影片，但您可以使用此 Markdown 擴充功能達成此目的。
...
在進行機器翻譯期間，系統會勾選標示為 &grave;&grave; 的項目，並根據本地化資料庫來決定適當的翻譯。 如果UI未本地化，此標籤將允許系統針對該特定語言保留英文UI參考（即義大利文的Analytics參考）。
...
我們使用「不要翻譯」列表來設定規則，讓機器翻譯引擎知道哪些應該保留英文內容。 最常見的項目為解決方案的完整名稱，如「Adobe Analytics」、「Adobe Campaign」和「Adobe Target」。 但是，可能有我們需要強制引擎使用英文的情況，因為所考慮的名詞可能是用作特定專業用語或是一般用語。 最明顯的例子是解決方案的簡短名稱，例如「Analytics」、「Campaign」、「Target」等。機器很難理解這些是解決方案名稱而不是一般術語。 該標記也可以用作一定要保留為英文或文字較短部分的第三方名稱/特性，例如必須保留為英文的短語或句子。
...
* [Markdown簡介](https://daringfireball.net/projects/markdown/syntax)
* [GitHub提供的Markdown基本介紹](https://help.github.com/articles/markdown-basics/)
