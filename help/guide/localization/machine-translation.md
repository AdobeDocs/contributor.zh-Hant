...
1. 按一下「語言選取器」(世界圖示)，然後選擇語言。
   ...
   內容可能會自動以您的語言顯示，或者，您可以按一下連結以觸發翻譯。 (當您按一下超連結從翻譯的頁面返回英文來源頁面時，可能會發生此情形。 這可讓您隨選檢視翻譯頁面。) 您隨時可以返回英文來源頁面。
...
您偶爾可能會遇到自動翻譯輸出的問題，例如翻譯不完整、字元損毀，或頁面版面問題。 這些問題是因 Adobe 使用的各種編寫、發佈或翻譯技術工具所造成。 我們會盡一切努力在這裡列出已知問題
底下。
...
內容經自動翻譯後，無論是因預設設定或使用者動作，您都可以選擇向 Adobe 提供翻譯相關的意見反應。 在
頁面頂端的淺藍色水準列，您可以針對「此翻譯對您是否有所幫助？」問題，回答「是」或「否」。 Adobe收集這項寶貴資訊
量度並分析編譯的資料，以決定內容翻譯。 我們重視您的意見反應，並歡迎您回應。

![意見反應](assets/machine-translation-feedback.png)

未來，我們計畫允許您以記錄問題、提出改進建議，甚至請求的方式，提供更廣泛的意見反應
其他語言。 英文頁面已提供上述部分功能。 長遠來看，我們認為這有助於Adobe改善使用者
在這些頁面上的體驗。
